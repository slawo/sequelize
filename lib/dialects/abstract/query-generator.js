...
var isInnerJoin = function (include) {
  return include.required ? 'left' !== (''+include.required).toLowerCase() : false;
};
    ...
    if (options.include) {
      var generateJoinQueries = function(include, parentTable) {
        var table = include.model.getTableName()
          ...
          , joinType = isInnerJoin(include) ? ' INNER JOIN ' : ' LEFT OUTER JOIN '
